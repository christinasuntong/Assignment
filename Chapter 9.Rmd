...
```{r}
library(e1071)
model <- svm(as.factor(y) ~ x1 + x2, data, kernel = "linear", cost = 0.01)
...
```{r}
set.seed(1)
tune.out <- tune(e1071::svm, mpglevel ~ ., data = Auto, kernel = "polynomial", ranges = list(cost = c(0.01, 0.1, 1, 5, 10, 100), degree = c(2, 3, 4)))
summary(tune.out)
```
...
```
```{r}
trainingerror <- (70+64) / (421+64+70+245)
trainingerror
```
...
test.pred <- predict(svm.linear, OJ.test)
table(OJ.test$Purchase, test.pred)
testerror <- (31+15) / (153+15+31+71)
testerror
```
 
We may see that, with the best cost, the trainging error rate is 16.75% and the test error rate is 17.04%.
